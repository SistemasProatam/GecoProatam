<?php
// descargar_cotizacion.php â€” Lee cotizaciÃ³n de BD y genera el PDF
...
$id = (int)($_GET['id'] ?? 0);
if (!$id) { http_response_code(400); die('ID invÃ¡lido'); }
...
if (!$cot) { http_response_code(404); die('CotizaciÃ³n no encontrada'); }
...
// Conceptos desde JSON â€” clave puede ser 'descripcion' o 'desc'
...
if (is_array($conceptosRaw) && !empty($conceptosRaw)) {
    ...
    $_POST['desc']     = ['(Ver cotizaciÃ³n original)'];
    $_POST['unidad']   = [''];
    $_POST['cantidad'] = [1];
