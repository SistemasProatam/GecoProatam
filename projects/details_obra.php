            ...
            case 'guardar_conceptos':
                ...
                try {
                    ...
                    if (!empty($cids)) {
                        ...
                        foreach ($cids as $cid) {
                            $cid = (int)$cid;
                            if ($cid > 0) { $s2->bind_param("ii", $sc_id, $cid); $s2->execute(); }
                        }
                    }
                    $conn->commit();
...
<head>
  ...
  .sc-card:hover{box-shadow:0 6px 20px rgba(0,0,0,.10);transform:translateY(-2px)}
  ...
  .mv-blue{color:#1a60a8}.mv-amber{color:#b47800}.mv-green{color:#1a7555}.mv-red{color:#c0392b}.mv-purple{color:#6d28d9}
  .prog-wrap{height:5px;background:rgba(0,0,0,.07);border-radius:99px;overflow:hidden;margin-top:4px}
  .prog-fill{height:100%;border-radius:99px;background:linear-gradient(90deg,#3f7555,#5fbe8a);transition:width .6s cubic-bezier(.16,1,.3,1)}
  ...
  /* editor */
  .sc-editor-overlay{display:none;position:fixed;inset:0;background:rgba(15,23,42,.52);z-index:1060;align-items:center;justify-content:center;backdrop-filter:blur(4px)}
  .sc-editor-overlay.open{display:flex}
  .sc-editor-box{background:#fff;border-radius:16px;width:96%;max-width:940px;max-height:88vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,.22);animation:scFadeUp .28s cubic-bezier(.16,1,.3,1)}
  @keyframes scFadeUp{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}
  .sc-editor-head{padding:18px 22px 14px;border-bottom:1px solid rgba(0,0,0,.07);display:flex;align-items:center;justify-content:space-between;flex-shrink:0}
  .sc-editor-body{flex:1;overflow:hidden;display:grid;grid-template-columns:1fr 1fr}
  .sc-editor-col{display:flex;flex-direction:column;overflow:hidden;padding:14px}
  .sc-editor-col:first-child{border-right:1px solid rgba(0,0,0,.07)}
  .sc-editor-foot{padding:12px 22px;border-top:1px solid rgba(0,0,0,.07);display:flex;justify-content:space-between;align-items:center;flex-shrink:0;font-size:.75rem;color:#8fa3b8}
  .dd-head{font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:#8fa3b8;margin-bottom:8px;display:flex;align-items:center;gap:6px}
  .dd-cnt{background:rgba(0,0,0,.07);padding:1px 7px;border-radius:20px;font-size:.68rem;font-weight:600}
  .dd-cnt.green{background:rgba(63,117,85,.15);color:#3f7555}
  .dd-search{width:100%;padding:6px 10px;border:1px solid rgba(0,0,0,.1);border-radius:7px;font-size:.8rem;background:#f8f9fa;margin-bottom:7px;font-family:inherit}
  .dd-search:focus{outline:none;border-color:#3f7555}
  .dd-zone{flex:1;overflow-y:auto;padding:4px;border-radius:8px;min-height:260px;max-height:340px;transition:background .15s}
  .dd-zone.drag-over{background:rgba(63,117,85,.06);outline:2px dashed rgba(63,117,85,.35);outline-offset:-3px}
  .concept-chip{display:flex;align-items:center;gap:7px;padding:7px 9px;border-radius:7px;border:1px solid rgba(0,0,0,.07);background:#fff;cursor:grab;margin-bottom:4px;transition:box-shadow .15s,border-color .15s;user-select:none;font-size:.78rem}
  .concept-chip:hover{border-color:rgba(63,117,85,.35);box-shadow:0 2px 8px rgba(0,0,0,.08)}
  .concept-chip.dragging{opacity:.4}
  .concept-chip.already{border-color:rgba(109,40,217,.25);background:rgba(109,40,217,.04)}
  .chip-cat{flex-shrink:0;font-size:.6rem;font-weight:700;padding:2px 5px;border-radius:4px;background:rgba(63,117,85,.12);color:#3f7555;white-space:nowrap}
  .chip-cod{font-size:.65rem;color:#8fa3b8;font-weight:600;white-space:nowrap;flex-shrink:0}
  .chip-nom{flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:500;color:#0f172a}
  .chip-um{font-size:.62rem;color:#8fa3b8;flex-shrink:0}
  .chip-also{font-size:.62rem;color:#6d28d9;flex-shrink:0;font-style:italic;white-space:nowrap}
  .dd-empty{text-align:center;padding:30px 10px;color:#8fa3b8;font-size:.78rem}
  .dd-empty i{font-size:1.6rem;display:block;margin-bottom:6px;opacity:.4}
  /* modal sc */
  .sc-modal-overlay{display:none;position:fixed;inset:0;background:rgba(15,23,42,.5);z-index:1070;align-items:center;justify-content:center;backdrop-filter:blur(3px)}
  .sc-modal-overlay.open{display:flex}
  .sc-modal-box{background:#fff;border-radius:14px;padding:26px;width:100%;max-width:500px;box-shadow:0 16px 48px rgba(0,0,0,.18);position:relative;animation:scFadeUp .25s cubic-bezier(.16,1,.3,1)}
  .sc-modal-title{font-size:.98rem;font-weight:700;color:#0f172a;margin-bottom:16px;padding-bottom:10px;border-bottom:2px solid #3f7555}
  .sc-mc{position:absolute;top:14px;right:14px;background:none;border:none;color:#8fa3b8;font-size:17px;cursor:pointer}
  .sc-mc:hover{color:#e8445a}
  .sc-fg{display:grid;grid-template-columns:1fr 1fr;gap:12px}
  .sc-fgi{display:flex;flex-direction:column;gap:3px}
  .sc-fgi.full{grid-column:1/-1}
  .sc-lbl{font-size:.7rem;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:#8fa3b8;margin-bottom:3px}
  .sc-inp{background:#f8f9fa;border:1px solid rgba(0,0,0,.12);border-radius:7px;color:#0f172a;padding:7px 11px;font-family:inherit;font-size:.84rem;transition:border-color .2s;width:100%}
  .sc-inp:focus{outline:none;border-color:#3f7555}
  .sc-fa{display:flex;justify-content:flex-end;gap:8px;margin-top:16px}
  .anticipo-preview{background:rgba(63,117,85,.07);border:1px solid rgba(63,117,85,.2);border-radius:7px;padding:8px 12px;font-size:.8rem;color:#3f7555;font-weight:600;margin-top:4px;display:none}
  /* extraordinarios */
  .ext-overlay{display:none;position:fixed;inset:0;background:rgba(15,23,42,.5);z-index:1080;align-items:center;justify-content:center;backdrop-filter:blur(3px)}
  .ext-overlay.open{display:flex}
  .ext-box{background:#fff;border-radius:14px;padding:0;width:100%;max-width:580px;max-height:85vh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 16px 48px rgba(0,0,0,.18);animation:scFadeUp .25s cubic-bezier(.16,1,.3,1)}
  .ext-head{padding:18px 22px 14px;border-bottom:1px solid rgba(0,0,0,.07);display:flex;align-items:center;justify-content:space-between;flex-shrink:0}
  .ext-head-title{font-size:.95rem;font-weight:700;color:#0f172a}
  .ext-head-sub{font-size:.74rem;color:#8fa3b8;margin-top:2px}
  .ext-body{flex:1;overflow-y:auto;padding:18px 22px}
  .ext-foot{padding:14px 22px;border-top:1px solid rgba(0,0,0,.07);flex-shrink:0}
  .ext-form{background:#f8f9fa;border:1px solid rgba(0,0,0,.08);border-radius:10px;padding:14px;margin-bottom:18px}
  .ext-form-title{font-size:.74rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#3f7555;margin-bottom:10px}
  .ext-fg{display:grid;grid-template-columns:1fr 1fr;gap:10px}
  .ext-fgi{display:flex;flex-direction:column;gap:3px}
  .ext-fgi.full{grid-column:1/-1}
  .ext-list-title{font-size:.74rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8fa3b8;margin-bottom:10px}
  .ext-item{display:flex;align-items:flex-start;gap:12px;padding:10px 12px;border-radius:8px;border:1px solid rgba(0,0,0,.07);background:#fff;margin-bottom:8px}
  .ext-item-monto{font-family:'Outfit',sans-serif;font-size:.92rem;font-weight:700;color:#6d28d9;white-space:nowrap;flex-shrink:0;min-width:90px}
  .ext-item-info{flex:1;min-width:0}
  .ext-item-desc{font-size:.82rem;font-weight:500;color:#0f172a;margin-bottom:2px}
  .ext-item-fecha{font-size:.72rem;color:#8fa3b8}
  .ext-item-del{background:none;border:none;color:#8fa3b8;cursor:pointer;font-size:.9rem;padding:2px 6px;border-radius:5px;transition:color .15s,background .15s;flex-shrink:0}
  .ext-item-del:hover{color:#e8445a;background:rgba(232,68,90,.08)}
  .ext-total-bar{background:rgba(109,40,217,.07);border:1px solid rgba(109,40,217,.2);border-radius:8px;padding:10px 14px;display:flex;justify-content:space-between;align-items:center}
  .ext-total-lbl{font-size:.8rem;font-weight:600;color:#6d28d9}
  .ext-total-val{font-family:'Outfit',sans-serif;font-size:1rem;font-weight:700;color:#6d28d9}
  /* extraordinarios inline en card */
  .sc-ext-panel{margin-top:10px;border-top:1px solid rgba(109,40,217,.12);padding-top:10px;display:none}
  .sc-ext-panel.open{display:block}
  .sc-ext-toggle{width:100%;background:rgba(109,40,217,.06);border:1px solid rgba(109,40,217,.15);border-radius:7px;padding:6px 10px;font-size:.72rem;font-weight:600;color:#6d28d9;cursor:pointer;display:flex;align-items:center;justify-content:space-between;transition:background .15s;font-family:inherit}
  .sc-ext-toggle:hover{background:rgba(109,40,217,.12)}
  .sc-ext-toggle i.arrow{transition:transform .2s}
  .sc-ext-toggle.open i.arrow{transform:rotate(180deg)}
  .sc-ext-row{display:flex;align-items:flex-start;gap:10px;padding:7px 0;border-bottom:1px solid rgba(0,0,0,.05);font-size:.78rem}
  .sc-ext-row:last-child{border-bottom:none}
  .sc-ext-monto{font-weight:700;color:#6d28d9;white-space:nowrap;min-width:80px;font-family:'Outfit',sans-serif}
  .sc-ext-info{flex:1;min-width:0}
  .sc-ext-desc{color:#0f172a;font-weight:500}
  .sc-ext-fecha{color:#8fa3b8;font-size:.68rem;margin-top:1px}
  .sc-ext-del{background:none;border:none;color:#ccc;cursor:pointer;padding:2px 5px;border-radius:4px;transition:color .15s,background .15s;font-size:.78rem}
  .sc-ext-del:hover{color:#e8445a;background:rgba(232,68,90,.08)}
  .sc-total-real{margin-top:10px;background:linear-gradient(90deg,rgba(26,96,168,.07),rgba(63,117,85,.07));border:1px solid rgba(63,117,85,.18);border-radius:8px;padding:8px 12px;display:flex;justify-content:space-between;align-items:center}
  .sc-total-real-lbl{font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#1a7555}
  .sc-total-real-val{font-family:'Outfit',sans-serif;font-size:.95rem;font-weight:800;color:#1a7555}
  .sc-desc-badge{margin-top:6px;font-size:.73rem;color:#475569;line-height:1.4;padding:5px 8px;background:rgba(0,0,0,.03);border-radius:6px;border-left:2px solid #cbd5e1;white-space:pre-wrap;word-break:break-word}
  /* botones */
  .btn-sc-p{background:#3f7555;border:none;border-radius:7px;color:#fff;padding:8px 18px;font-family:inherit;font-size:.82rem;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:5px;transition:opacity .2s}
  .btn-sc-p:hover{opacity:.88}
  .btn-sc-p.purple{background:#6d28d9}
  .btn-sc-s{background:#f8f9fa;border:1px solid rgba(0,0,0,.13);border-radius:7px;color:#0f172a;padding:8px 18px;font-family:inherit;font-size:.82rem;cursor:pointer;display:inline-flex;align-items:center;gap:5px;transition:all .2s}
  .btn-sc-s:hover{border-color:#3f7555;color:#3f7555}
  .btn-sc-icon{background:none;border:1px solid rgba(0,0,0,.11);border-radius:6px;color:#8fa3b8;padding:4px 8px;cursor:pointer;font-size:.8rem;transition:all .15s;display:inline-flex;align-items:center;gap:3px}
  .btn-sc-icon:hover{border-color:#3f7555;color:#3f7555}
  .btn-sc-icon.danger:hover{border-color:#e8445a;color:#e8445a}
  .btn-sc-icon.purple:hover{border-color:#6d28d9;color:#6d28d9}
  #sc-toast{position:fixed;bottom:22px;right:22px;background:#fff;border-left:4px solid #3f7555;border-radius:9px;padding:11px 16px;font-size:.82rem;font-weight:500;max-width:320px;box-shadow:0 4px 20px rgba(0,0,0,.14);z-index:9999;transform:translateY(60px);opacity:0;transition:all .3s cubic-bezier(.16,1,.3,1);pointer-events:none;color:#0f172a}
  #sc-toast.show{transform:translateY(0);opacity:1}
  #sc-toast.error{border-color:#e8445a}
  #sc-toast.success{border-color:#3f7555}
  #sc-toast.info{border-color:#6d28d9}
...
<div class="section-header">
  ...
  <div class="works-section" style="margin-top:24px;">
    <div class="section-header">
      <div class="section-title-group"><h4>Subcontratos</h4></div>
    ...
    <div class="row g-2 mb-3" id="sc-stats" style="display:none!important">
      <div class="col-6 col-md-3"><div class="budget-stat"><div class="budget-stat-label">Subcontratos</div><div class="budget-stat-value" id="sc-stat-total">0</div></div></div>
      <div class="col-6 col-md-3"><div class="budget-stat"><div class="budget-stat-label">Total Monto Contratos</div><div class="budget-stat-value" id="sc-stat-est">$0</div></div></div>
      <div class="col-6 col-md-3"><div class="budget-stat"><div class="budget-stat-label">Total Anticipos</div><div class="budget-stat-value" id="sc-stat-ant">$0</div></div></div>
      <div class="col-6 col-md-3"><div class="budget-stat"><div class="budget-stat-label">Total Pagado</div><div class="budget-stat-value" id="sc-stat-real">$0</div></div></div>
    </div>
    <div id="sc-grid">
      <div class="text-center text-muted py-4">
...
<div class="sc-modal-overlay" id="sc-modal">
  <div class="sc-modal-box">
    ...
    <div class="sc-fg">
      <div class="sc-fgi full"><div class="sc-lbl">Proveedor</div><select id="sc-m-proveedor" class="sc-inp"><option value="">Selecciona proveedor</option></select></div>
      <div class="sc-fgi"><div class="sc-lbl">Total monto de contrato</div><input type="number" id="sc-m-total" class="sc-inp" placeholder="0.00" step="0.01" min="0" oninput="scCalcAnticipo()"></div>
      <div class="sc-fgi"><div class="sc-lbl">Anticipo (%)</div><input type="number" id="sc-m-pct" class="sc-inp" placeholder="0" step="0.1" min="0" max="100" oninput="scCalcAnticipo()"></div>
      <div class="sc-fgi full"><div class="anticipo-preview" id="sc-ant-preview"><i class="bi bi-arrow-right-circle me-1"></i>Anticipo en pesos: <strong id="sc-ant-monto">$0.00</strong></div></div>
      <div class="sc-fgi full"><div class="sc-lbl">Descripcion del subcontrato</div><textarea id="sc-m-desc" class="sc-inp" rows="2" placeholder="Descripcion general del alcance o trabajos..."></textarea></div>
    </div>
...
<div class="ext-overlay" id="ext-modal">
  <div class="ext-box">
    <div class="ext-head">
      <div><div class="ext-head-title" id="ext-modal-title">Valores extraordinarios</div><div class="ext-head-sub" id="ext-modal-sub"></div></div>
    ...
    <div class="ext-body">
      <div class="ext-form">
        ...
        <div class="ext-fg">
          <div class="ext-fgi"><div class="sc-lbl">Monto</div><input type="number" id="ext-monto" class="sc-inp" placeholder="0.00" step="0.01"></div>
          <div class="ext-fgi"><div class="sc-lbl">Fecha</div><input type="date" id="ext-fecha" class="sc-inp"></div>
          <div class="ext-fgi full"><div class="sc-lbl">Descripcion</div><input type="text" id="ext-desc" class="sc-inp" placeholder="Ej: Trabajos adicionales..."></div>
        </div>
      ...
      <div id="ext-lista"><div class="dd-empty"><i class="bi bi-inbox"></i>Sin registros extraordinarios</div></div>
    </div>
    <div class="ext-foot">
...
var scFmt=function(n){return new Intl.NumberFormat('es-MX',{style:'currency',currency:'MXN'}).format(n||0);};
var scEl=function(id){return document.getElementById(id);};

function scToast(msg,type){
  type=type||'success';var t=scEl('sc-toast');
...
function scAjax(data,cb){
  ...
  fetch('details_obra.php?id='+SC_OBRA_ID,{method:'POST',body:fd})
    .then(function(r){return r.json();}).then(function(r){cb(null,r);})
    .catch(function(){cb(null,{success:false,error:'Error de red'});});
}

document.addEventListener('DOMContentLoaded',function(){
  if(!SC_OBRA_ID)return;
  scEl('ext-fecha').value=new Date().toISOString().split('T')[0];
  scAjax({action:'obtener_proveedores'},function(e,rP){
    if(rP.success){scProveedores=rP.proveedores;scPoblarSelect();}
    scAjax({action:'obtener_conceptos_obra',obra_id:SC_OBRA_ID},function(e2,rC){
...
function scPoblarSelect(){
  ...
  scProveedores.forEach(function(p){var o=document.createElement('option');o.value=p.id;o.textContent=p.nombre;sel.appendChild(o);});
}

function scCargar(){
  scAjax({action:'obtener_subcontratos',obra_id:SC_OBRA_ID},function(e,r){
    if(!r.success){scToast('Error al cargar subcontratos','error');return;}
    scLista=r.subcontratos;scRender();scStats();
  });
}
...
function scToggleExt(scId){
  var panel=scEl('sc-ext-'+scId);if(!panel)return;
  panel.classList.toggle('open');
}

function scCargarExtInline(scId){
  scAjax({action:'obtener_extraordinarios',subcontrato_id:scId},function(e,r){
    var rows=scEl('sc-ext-rows-'+scId);if(!rows)return;
    if(!r.success||!r.extraordinarios.length){rows.innerHTML='<div style="font-size:.75rem;color:#8fa3b8;padding:4px 0;">Sin registros aun</div>';return;}
    rows.innerHTML=r.extraordinarios.map(function(x){
      var fd=x.fecha?new Date(x.fecha+'T00:00:00').toLocaleDateString('es-MX',{day:'2-digit',month:'short',year:'numeric'}):'';
...
function extEliminarInline(id,scId){
  ...
  scAjax({action:'eliminar_extraordinario',id:id,subcontrato_id:scId},function(e,r){
    if(!r.success){scToast(r.error||'Error','error');return;}
    scToast('Extraordinario eliminado');
    var sc=scLista.find(function(x){return x.id==scId;});
    if(sc){sc.total_extraordinarios=r.total_extraordinarios;sc.num_extraordinarios=r.num_extraordinarios;}
    scRender();
  });
}
...
function scStats(){
  ...
  if(!scLista.length){wrap.style.setProperty('display','none','important');return;}
  wrap.style.setProperty('display','flex','important');
  scEl('sc-stat-total').textContent=scLista.length;
  scEl('sc-stat-est').textContent = scFmt(scLista.reduce(function(s, x) { return s + (parseFloat(x.total_estimado) || 0) + (parseFloat(x.total_extraordinarios) || 0);}, 0));
  scEl('sc-stat-ant').textContent=scFmt(scLista.reduce(function(s,x){return s+parseFloat(x.anticipo_monto||0);},0));
  scEl('sc-stat-real').textContent=scFmt(scLista.reduce(function(s,x){return s+parseFloat(x.monto_real||0);},0));
}

function scAbrirModal(){scEl('sc-m-id').value='';scEl('sc-modal-title').textContent='Nuevo subcontrato';scEl('sc-m-proveedor').value='';scEl('sc-m-total').value='';scEl('sc-m-pct').value='';scEl('sc-m-desc').value='';scEl('sc-ant-preview').style.display='none';scEl('sc-modal').classList.add('open');}
function scEditar(id){
  var sc=scLista.find(function(x){return x.id==id;});if(!sc)return;
  scEl('sc-m-id').value=sc.id;scEl('sc-modal-title').textContent='Editar subcontrato';
  scEl('sc-m-proveedor').value=sc.proveedor_id||'';scEl('sc-m-total').value=sc.total_estimado;
  scEl('sc-m-pct').value=sc.anticipo_pct;
  scEl('sc-m-desc').value=sc.descripcion||'';
  scCalcAnticipo();scEl('sc-modal').classList.add('open');
}
function scCerrarModal(){scEl('sc-modal').classList.remove('open');}
function scCalcAnticipo(){
  var total=parseFloat(scEl('sc-m-total').value)||0;var pct=parseFloat(scEl('sc-m-pct').value)||0;
  var prev=scEl('sc-ant-preview');
  if(total>0&&pct>0){prev.style.display='block';scEl('sc-ant-monto').textContent=scFmt(total*pct/100);}
...
function scEjecutarGuardar(id, proveedor_id, total, pct, desc){
    var payload = id
        ? {action:'actualizar_subcontrato', id:id, proveedor_id:proveedor_id, total_estimado:total, anticipo_pct:pct, descripcion:desc}
        : {action:'crear_subcontrato', obra_id:SC_OBRA_ID, proveedor_id:proveedor_id, total_estimado:total, anticipo_pct:pct, descripcion:desc};
    scAjax(payload, function(e, r){
        if(!r.success){ scToast(r.error || 'Error al guardar', 'error'); return; }
...
function scEliminar(id){
  var sc=scLista.find(function(x){return x.id==id;});if(!sc)return;
  Swal.fire({title:'Eliminar subcontrato?',text:'Se eliminaran los conceptos asignados y los valores extraordinarios.',icon:'warning',showCancelButton:true,confirmButtonText:'Si, eliminar',cancelButtonText:'Cancelar',confirmButtonColor:'#dc3545'})
    .then(function(conf){
      if(!conf.isConfirmed)return;
      scAjax({action:'eliminar_subcontrato',id:id},function(e,r){
        if(r.success){scToast('Subcontrato eliminado');scCargar();}
...
function scAbrirEditor(id){
  ...
  scAjax({action:'obtener_conceptos_obra',obra_id:SC_OBRA_ID},function(e,r){
    if(!r.success){scToast('Error al cargar conceptos','error');return;}
    ...
    scEl('sc-search-disp').value='';scEl('sc-search-asig').value='';
    scRenderZonas();scEl('sc-editor').classList.add('open');
  });
}
function scCerrarEditor(){scEl('sc-editor').classList.remove('open');scEditorId=null;}
...
function scRenderZona(zoneId,lista,filtro,esAsig){
  var zone=scEl(zoneId);var q=filtro.toLowerCase().trim();
  var vis=q?lista.filter(function(c){return c.nombre_concepto.toLowerCase().indexOf(q)!==-1||c.codigo_concepto.toLowerCase().indexOf(q)!==-1;}):lista;
  if(!vis.length){zone.innerHTML='<div class="dd-empty"><i class="bi bi-'+(esAsig?'inbox':'check2-all')+'"></i>'+(esAsig?'Arrastra conceptos aqui':'Sin conceptos disponibles')+'</div>';return;}
  zone.innerHTML=vis.map(function(c){
    var enOtro=!esAsig&&c.subcontratos_ids&&c.subcontratos_ids.split(',').some(function(sid){return sid!==''&&parseInt(sid)!==scEditorId;});
...
function scOnDragStart(e,id){scDragId=id;e.target.classList.add('dragging');e.dataTransfer.effectAllowed='move';}
function scOnDragEnd(e){e.target.classList.remove('dragging');scDragId=null;}
...
function scOnDrop(e,destino){e.preventDefault();e.currentTarget.classList.remove('drag-over');if(!scDragId)return;scMoverConcepto(scDragId,destino);}
function scMoverConcepto(id,destino){
  id=parseInt(id);
  if(destino==='asig'){var idx=scDisp.findIndex(function(c){return c.id===id;});if(idx===-1)return;var m=scDisp.splice(idx,1)[0];scAsig.push(m);}
  else{var idx2=scAsig.findIndex(function(c){return c.id===id;});if(idx2===-1)return;var m2=scAsig.splice(idx2,1)[0];scDisp.push(m2);}
  scRenderZonas();
}
function scFiltrarDisp(){scRenderZona('sc-zone-disp',scDisp,scEl('sc-search-disp').value,false);}
function scFiltrarAsig(){scRenderZona('sc-zone-asig',scAsig,scEl('sc-search-asig').value,true);}
function scGuardarConceptos(){
  if(!scEditorId){scToast('Sin subcontrato activo','error');return;}
  var btn=scEl('sc-btn-guardar-conc');btn.disabled=true;
  scAjax({action:'guardar_conceptos',subcontrato_id:scEditorId,concepto_ids:JSON.stringify(scAsig.map(function(c){return c.id;}))},
    function(e,r){btn.disabled=false;if(r.success){scToast(r.total+' conceptos guardados');scCerrarEditor();scCargar();}else scToast(r.error||'Error al guardar','error');});
}

/* EXTRAORDINARIOS */
function extAbrir(scId){
  extScId=scId;var sc=scLista.find(function(x){return x.id==scId;});
  ...
  scEl('ext-monto').value='';scEl('ext-desc').value='';
  scEl('ext-fecha').value=new Date().toISOString().split('T')[0];
  extCargar();scEl('ext-modal').classList.add('open');
}
function extCerrar(){scEl('ext-modal').classList.remove('open');extScId=null;}
function extCargar(){
  scAjax({action:'obtener_extraordinarios',subcontrato_id:extScId},function(e,r){
    if(!r.success){scToast('Error al cargar','error');return;}
    extLista=r.extraordinarios;extRender();
  });
}
function extRender(){
  var lista=scEl('ext-lista');
  var total=extLista.reduce(function(s,x){return s+parseFloat(x.monto||0);},0);
  scEl('ext-total').textContent=scFmt(total);
  if(!extLista.length){lista.innerHTML='<div class="dd-empty"><i class="bi bi-inbox"></i>Sin registros extraordinarios</div>';return;}
...
function extAgregar(){
  ...
  if(!monto){scToast('Ingresa un monto','error');return;}
  if(!desc){scToast('Ingresa una descripcion','error');return;}
  if(!fecha){scToast('Ingresa una fecha','error');return;}
  scAjax({action:'agregar_extraordinario',subcontrato_id:extScId,monto:monto,descripcion:desc,fecha:fecha},
    function(e,r){
      if(!r.success){scToast(r.error||'Error al agregar','error');return;}
      scToast('Extraordinario registrado','success');
      scEl('ext-monto').value='';scEl('ext-desc').value='';
      var sc=scLista.find(function(x){return x.id==extScId;});
      if(sc){sc.total_extraordinarios=r.total_extraordinarios;sc.num_extraordinarios=r.num_extraordinarios;}
      extCargar();scRender();
    });
}
function extEliminar(id){
  Swal.fire({title:'Eliminar este registro?',icon:'warning',showCancelButton:true,confirmButtonText:'Si, eliminar',confirmButtonColor:'#dc3545'})
    .then(function(conf){
      if(!conf.isConfirmed)return;
      scAjax({action:'eliminar_extraordinario',id:id,subcontrato_id:extScId},function(e,r){
        if(!r.success){scToast(r.error||'Error','error');return;}
        scToast('Registro eliminado');
        var sc=scLista.find(function(x){return x.id==extScId;});
        if(sc){sc.total_extraordinarios=r.total_extraordinarios;sc.num_extraordinarios=r.num_extraordinarios;scRender();}
        extCargar();
      });
    });
}
